...
class PolynomialRegression() :
    ...
    def fit_GD(self, X, y, eta=None,
        ...
        for t in xrange(tmax) :
            ...
            self.coef_ -= (np.dot(np.dot(X.transpose(), X), self.coef_) 
                           - np.dot(X.transpose(), y))*eta
            # track error
    ...
    def fit(self, X, y, l2regularize = None ) :
        ...
        start = time.time()
        self.coef_ = np.linalg.pinv(np.dot(X.transpose(), X)).dot(X.transpose()).dot(y)
        end = time.time()
        print 'time elapsed: %f' % (end - start)
...
def main() :
    ...
    linear3.fit_GD(train_data.X,train_data.y, 0.001)
    print linear3.coef_
    ...
    plt.plot(range(0,11), errors[:, 0], 'b--', label='training error')
