...
def kMeans(points, k, init='random', plot=False, verbose = False) :
    ...
    while True:
        ...
        #if plot: plot_clusters(cur_cluster_set, 'kMeans, iteration {}, using {} init'.format(iters, init),
        ...
        if prev_clusts is not None and cur_cluster_set.equivalent(prev_clusts):
            ...
            if plot: plot_clusters(cur_cluster_set, 'kMeans converged, iteration {} (unchanged from previous), using {} init'.format(iters, init),
                               ClusterSet.centroids)
    ...
    while True:
        ...
        cur_cluster_set = ClusterSet([Cluster(v) for _, v
                                      in compute_assignments_kMeans(
                                          cur_centroids, points).items()])
        iters+=1
        #if plot: plot_clusters(cur_cluster_set, 'KMediods, iteration {}, using {} init'.format(iters, init),
                               #ClusterSet.medoids)
        if verbose: print "iters: {}".format(iters)
        if prev_clusts is not None and cur_cluster_set.equivalent(prev_clusts):
            if plot: plot_clusters(cur_cluster_set, 'KMediods converged, iteration {} (unchaged from previous), using {} init'.format(iters, init),
                               ClusterSet.medoids)
            if verbose: print "Done in {} iters".format(iters)
            return cur_cluster_set
        else:
...
def main() :
    ...
    cluster_set = kMeans(pts, 3, plot = True, verbose = False) # 2
    print "kmeans rand init score: {}".format(cluster_set.score())
    another_cluster_set = kMedoids(pts, 3, plot = True, verbose = False) #2
    print "k medoids rand init score: {}".format(another_cluster_set.score())
    km_clust_2 = kMeans(pts, 3, init = 'cheat', plot = True, verbose = False) #2
    print "k means cheat init score: {}".format(km_clust_2.score())
    k_med_clust_2 = kMedoids(pts, 3, init='cheat', plot = True, verbose = False) #2
    ...
    plt.title('K-means/K-medoids performance vs principal components')
