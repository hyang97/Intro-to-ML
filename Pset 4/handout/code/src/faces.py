...
def kMeans(points, k, init='random', plot=False) :
    ...
    if init == 'random':
        ...
        print "Use either 'cheat' or 'random' as initialization method"
        return

    #step 3: repeat step 1 and 2 until convergence
    prev_clusters = cur_centroids
...
def main() :
    ...
    #import faces
    features, targets = get_lfw_data()

    #perform PCA (eigendecomposition)
    eigenfaces, mean_features = PCA(features)

    #Problem 1A
    show_image(features[3])
    show_image(features[7])
    show_image(mean_features)

    #Problem 1B
    show_image(vec_to_image(eigenfaces[:,100]))
    plot_gallery([vec_to_image(eigenfaces[:,i]) for i in xrange(12)])

    #Problem 1C
    l_values = [1, 10, 50, 100, 500, 1288]
    for l in l_values:
        print l
        lower_features, used_eigenfaces = apply_PCA_from_Eig(features, eigenfaces, l, mean_features)
        reconstructed_features = reconstruct_from_PCA(lower_features, used_eigenfaces, mean_features)
        plot_gallery(reconstructed_features[0:12,:])
    """
    ### ========== TODO : END ========== ###
